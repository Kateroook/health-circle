...
const MainStatusIndicator = ({
    ...
    const handleLongPress = () => {
        ...
        Alert.alert(
            ...
            [
                { text: "Скасувати", style: "cancel" },
                { text: "ТАК, ПОТРІБНА ДОПОМОГА", style: "destructive", onPress: () => onUpdateStatus("DANGER") },
            ]
        );
    };
...
const StatusBadge = ({ status }: { status: UserStatus }) => {
    ...
    switch (status) {
        case "SAFE":
            bgColor = "#E8F5E9"; circleColor = "#4CAF50"; symbol = "✓"; break;
        case "UNKNOWN":
            bgColor = "#FFF3E0"; circleColor = "#FF9800"; symbol = "?"; break;
        case "DANGER":
            bgColor = "#FFEBEE"; circleColor = "#F44336"; symbol = "!"; break;
        default:
            bgColor = "#F5F5F5"; circleColor = "#9E9E9E"; symbol = "?";
    }

    return (
        <View style={{ width: 30, height: 30, borderRadius: 5, backgroundColor: bgColor, justifyContent: "center", alignItems: "center" }}>
            <View style={{ width: 20, height: 20, borderRadius: 10, backgroundColor: circleColor, justifyContent: "center", alignItems: "center" }}>
                <Text style={{ color: "#FFFFFF", fontSize: 14, fontWeight: "bold", includeFontPadding: false, lineHeight: 20 }}>
                    {symbol}
                </Text>
            </View>
        </View>
...
const ContactStatusRow = ({
    ...
    switch (member.status) {
        case "SAFE": statusText = "В безпеці"; statusColor = "#4CAF50"; break;
        case "DANGER": statusText = "Потрібна допомога!"; statusColor = "#F44336"; break;
        default: statusText = "Невідомо"; statusColor = "#FF9800";
    }
    ...
    return (
        <TouchableOpacity style={styles.contactRow} activeOpacity={0.7} onPress={() => onPress(member)}>
            ...
            <View style={styles.contactInfo}>
                <Text style={styles.contactName}>{member.firstName} {member.lastName}</Text>
...
export default function DashboardScreen() {
    ...
    return (
        <SafeAreaView style={styles.screen} edges={["top", "left", "right"]}>
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.statusCircleSection}>
                    ...
                    >
                        ...
                        >
                            <Text style={selectedGroupId === "ALL" ? styles.statusFilterTextActive : styles.statusFilterText}>
                                Усі
                            </Text>
                        </TouchableOpacity>
                        ...
                        {groups.map((group) => (
                            <TouchableOpacity
                                key={group.id}
                                style={selectedGroupId === group.id ? styles.statusFilterActive : styles.statusFilter}
                                onPress={() => setSelectedGroupId(group.id)}
                            >
                                <Text numberOfLines={1} style={selectedGroupId === group.id ? styles.statusFilterTextActive : styles.statusFilterText}>
                                    {group.name}
                                </Text>
                            </TouchableOpacity>
                        ))}
            ...
            <MemberProfileModal member={selectedMember} visible={modalVisible} onClose={handleCloseModal} />
        </SafeAreaView>
    );
}
