...
test.describe('DELETE /api/users cleanup', async () => {
  test('[USR-050] Deleting a member removes them from circle members', async ({ spawnApi, spawnUser }) => {
  ...
  test('[USR-051] Deleting an owner removes owned circles for remaining members', async ({ spawnApi, spawnUser }) => {
    const owner = await spawnUser();
    const member = await spawnUser();
