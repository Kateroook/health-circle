...
test.describe(
  ...
  async () => {
    ...
    test(
      '[USR-032] Getting an existing user by id',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
    ...
    //має повертати 404
    test(
      '[USR-033] Getting user by non-existent id',
      {
        tag: ['@sanity', '@bug'],
      },
      async ({ api }) => {
    ...
    test(
      '[USR-034] Getting an existing user by invalid format of id',
      {
        tag: '@sanity',
      },
      async ({ api }) => {
    ...
    test(
      '[USR-035] Getting a user by id without authorization',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
