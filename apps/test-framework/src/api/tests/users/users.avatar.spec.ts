...
test.describe(
  ...
  async () => {
    ...
    test(
      '[USR-056] Avatar uploading in PNG format',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
        const avatar = utils.random.avatar('.png');
        const userWithAvatar = await api.users.uploadUserAvatar(user.id!, avatar);
    ...
    test(
      '[USR-057] Avatar uploading in JPG format',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
        const avatar = utils.random.avatar('.jpg');
        const userWithAvatar = await api.users.uploadUserAvatar(user.id!, avatar);
        expect(userWithAvatar.response).toHaveStatus(200);
        expect(userWithAvatar.data).toMatchObject({
          id: user.id,
    ...
    test(
      '[USR-058] Getting an avatar after uploading',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
        const avatarFormat = utils.random.pick(['.png', '.jpg'] as const);
    ...
    test(
      '[USR-059] Avatar deletion',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
        const avatar = utils.random.avatar(utils.random.pick(['.png', '.jpg']));
        const userWithAvatar = await api.users.uploadUserAvatar(user.id!, avatar);
        expect(userWithAvatar.response).toHaveStatus(200);
    ...
    test(
      '[USR-060] Getting avatar after deletion',
      {
        tag: '@sanity',
      },
      async ({ api }) => {
    ...
    test(
      '[USR-061] Avatar uploading for someone else`s id',
      {
        tag: '@sanity',
      },
      async ({ api, spawnUser }) => {
        ...
        const avatar = utils.random.avatar(utils.random.pick(['.png', '.jpg']));
        const userWithAvatar = await api.users.uploadUserAvatar(userB.id!, avatar);
        expect(userWithAvatar.response).toHaveStatus4xx;
      },
    );

    test(
      '[USR-062] Avatar uploading without authorization',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
