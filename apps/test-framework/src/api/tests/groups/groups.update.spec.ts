...
test.describe(
  ...
  async () => {
    ...
    test(
      '[GRP-014] Successful group rename by owner',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
    ...
    ].forEach(({ testName, payloadOverride, tag }) => {
      test(
        testName,
        {
          tag: tag,
    ...
    test.fixme(
      '[GRP-019-BUG] Update group without members field',
      {
        tag: '@sanity',
      },
      async ({ api }) => {
    ...
    test(
      '[GRP-020] Update group by non-owner member',
      {
        tag: '@sanity',
      },
      async ({ api, spawnUser }) => {
    ...
    test(
      '[GRP-021] Update group without authorization',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
