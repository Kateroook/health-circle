...
test.describe('api/groups/invite tests', async () => {
  ...
  test('[GRP-041] Successful invite code regeneration by owner', async ({ api }) => {
  ...
  test('[GRP-042] Join group with outdated invite code', async ({ api, spawnApi, spawnUser }) => {
  ...
  test('[GRP-043] Invite code regeneration by non-owner member', async ({ spawnApi, spawnUser }) => {
    const secondUser = await spawnUser();
    const secondApi = await spawnApi();
    await secondApi.auth.login({
      identifier: secondUser.email,
  ...
  test('[GRP-044] Invite code regeneration without authorization', async ({ api }) => {
