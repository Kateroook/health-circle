...
import {TestContext } from '../../core/api/helpers/test-context'
...
test.describe("Auth API Tests", () => {
  ...
  test("Отримання профілю авторизованого користувача", async ({ request }) => {
    ...
    const response = await authWithAccessToken({
      request: request,
  ...
  test("Logout авторизованого користувача", async ({ request }) => {
    ...
    // Logout
    const response = await request.post(`${config.baseApiUrl}/api/auth/logout`, {
      headers: {
