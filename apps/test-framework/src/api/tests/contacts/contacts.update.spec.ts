...
test.describe(
  ...
  async () => {
    ...
    test(
      '[CNT-017] Successful contact update',
      {
        tag: '@smoke',
      },
    ...
    test(
      '[CNT-018] Contact update with non-existing target ID',
      {
        tag: '@sanity',
      },
      async ({ api }) => {
    ...
    test(
      '[CNT-019] Contact update without authorization',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
