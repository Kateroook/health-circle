...
test.describe("Users API Tests", () => {
  ...
  //     const response = await request.post(`${config.baseApiUrl}/api/users`, {
  //       data: {
  ...
  test("Створення користувача з дублюючим email", async ({ request }) => {
    ...
    // Перше створення
    await request.post(`${config.baseApiUrl}/api/users`, {
      data: {
        email: testEmail,
        phone: "+380501234567",
        firstName: "Тест",
  ...
  test("Отримання користувача за ID з невалідним UUID", async ({ request }) => {
    const response = await request.get(
      `${config.baseApiUrl}/api/users/invalid-uuid`,
      {
        headers: {
  ...
  test("Отримання користувача за неіснуючим ID", async ({ request }) => {
    ...
    const response = await request.get(
      `${config.baseApiUrl}/api/users/${fakeUUID}`,
      {
        headers: {
          Authorization: `Bearer fake-token`,
        },
