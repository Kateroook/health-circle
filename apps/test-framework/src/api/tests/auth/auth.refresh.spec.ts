...
test.describe(
  ...
  async () => {
    ...
    test(
      '[AUTH-012] Refresh with access token instead of refresh token',
      {
        tag: '@smoke',
      },
      async ({ api, spawnUser }) => {
    ...
    test(
      '[AUTH-013] Refresh without token',
      {
        tag: '@sanity',
      },
      async ({ api, spawnUser }) => {
    ...
    test(
      "[AUTH-014] Old access token doesn't work after refresh",
      {
        tag: '@smoke',
      },
      async ({ api, spawnUser }) => {
    ...
    test(
      "[AUTH-015] Old refresh token doesn't work after refresh",
      {
        tag: '@smoke',
      },
      async ({ api, spawnUser }) => {
