...
test.describe(
  'api/auth/change-password tests',
  {
    tag: '@auth',
  },
  async () => {
    test(
      '[AUTH-041] Forgot password request for existing email',
      {
        tag: '@smoke',
      },
      async ({ api, spawnUser, confirmationCodeRepository }) => {
    ...
    test(
      '[AUTH-042] Forgot password request for not registered email',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
    ...
    test(
      '[AUTH-043] Forgot password request for incorrect email',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
    ...
    test(
      '[AUTH-044] Forgot password request for an empty email',
      {
        tag: '@smoke',
      },
      async ({ api }) => {
